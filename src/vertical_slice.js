...
class Sfx{
  constructor(scene){this.s=scene;this.muted=false;this.master=2.35;this.last={}}
  ...
  pop(chain=1){if(!this.can('pop',20))return;const p=1+Math.min(chain,5)*.04+(Math.random()-.5)*.08;this.tone(210*p,.07,.027,'sine',95);this.tone(420*p,.045,.009,'triangle',120,.012);if(chain>=3&&Math.random()>.35)this.tone(900+chain*45,.08,.008,'sine',190,.015)}
...
class Play extends Phaser.Scene{
  ...
  create(){
    ...
    this.goals=this.cfg.g.map(x=>({type:x[0],id:x[1],need:x[2],done:0}));this.inventory={hammer:2,shuffle:2,fan:2};this.board=Array.from({length:R},()=>Array(C).fill(null));this.cell=Array.from({length:R},()=>Array.from({length:C},()=>({ice:0,block:null,over:null})));this.spr=Array.from({length:R},()=>Array(C).fill(null));
  ...
  async clearCells(initial,chain=1){const set=this.expandSpecials(new Set(initial)),damageRoots=new Set(),damageAcorns=new Set();for(const key of set){const [r,c]=key.split(',').map(Number),it=this.board[r][c];if(it?.sp==='line_h'){this.fx.whoosh();this.lineFx(r,c,'h')}if(it?.sp==='line_v'){this.fx.whoosh();this.lineFx(r,c,'v')}if(it?.sp==='bomb'){this.fx.bomb();this.bombFx(r,c)}if(it?.sp==='rainbow')this.rainbowFx();for(const [dr,dc] of [[1,0],[-1,0],[0,1],[0,-1]]){const rr=r+dr,cc=c+dc;if(!inBounds(rr,cc))continue;if(this.cell[rr][cc].block==='roots')damageRoots.add(`${rr},${cc}`);if(this.cell[rr][cc].block==='acorn')damageAcorns.add(`${rr},${cc}`)}}
    for(const key of set){const [r,c]=key.split(',').map(Number),it=this.board[r][c],ce=this.cell[r][c];if(!it)continue;const p=this.pos(r,c);if(ce.ice>0){ce.ice--;this.bumpGoal('ice',null,1);this.fx.crack();this.burst(p.x,p.y,0xbdeeff,10);this.render(r,c);continue}this.bumpGoal('berry',it.id,1);this.score+=50*Math.min(2,1+(chain-1)*.25);this.fx.pop(chain);this.burst(p.x,p.y,0xffd66f,6);const s=this.spr[r][c];if(s)await new Promise(z=>this.tweens.add({targets:s,scaleX:s.scaleX*1.22,scaleY:s.scaleY*.68,alpha:0,duration:145,ease:'Quad.in',onComplete:z}));this.board[r][c]=null;this.render(r,c)}
    ...
    for(const key of damageAcorns){const [r,c]=key.split(',').map(Number);if(this.cell[r][c].block==='acorn'&&!this.cell[r][c].ice){this.cell[r][c].block=null;this.bumpGoal('acorn',null,1);this.fx.spark();const p=this.pos(r,c);this.burst(p.x,p.y,0xffe48d,10);this.render(r,c)}}this.updateHud();if(chain>=2)this.kingReact('celebrate',520)}
  ...
  async useBoosterAt(r,c){const id=this.boosterMode;if(!id||this.inventory[id]<=0)return;this.busy=true;this.boosterMode=null;this.boosterHint.setText('');if(id==='hammer'){this.inventory.hammer--;const ce=this.cell[r][c],p=this.pos(r,c);if(ce.ice>0){ce.ice--;this.bumpGoal('ice',null,1);this.fx.crack();this.burst(p.x,p.y,0xbdeeff,12)}else if(ce.block==='roots'){ce.block=null;this.bumpGoal('roots',null,1);this.fx.wood();this.burst(p.x,p.y,0x9c6b3c,12)}else if(ce.block==='acorn'){ce.block=null;this.bumpGoal('acorn',null,1);this.fx.spark();this.burst(p.x,p.y,0xffe48d,12)}else if(this.board[r][c]){const it=this.board[r][c];this.bumpGoal('berry',it.id,1);this.board[r][c]=null;this.fx.pop(1);this.burst(p.x,p.y,0xffd66f,10)}this.render(r,c);await this.fallRefill();await this.resolve()}else if(id==='fan'){this.inventory.fan--;const set=new Set();for(let cc=0;cc<C;cc++)set.add(`${r},${cc}`);this.fx.whoosh();this.lineFx(r,c,'h');await this.clearCells(set,2);await this.fallRefill();await this.resolve()}this.busy=false;this.refreshBoosters();this.updateHud();this.endCheck();this.scheduleHint()}
  async useShuffle(){if(this.busy||this.inventory.shuffle<=0)return;this.busy=true;this.inventory.shuffle--;for(let k=0;k<30;k++){this.shuffleBoard();if(this.groups().length===0&&this.hasMove())break}this.fx.whoosh();this.drawAll(false,true);await pause(600);this.busy=false;this.refreshBoosters();this.scheduleHint()}
  ...
  async win(){if(this.busy)return;this.busy=true;clearTimeout(this.hintTimer);window.__berriesGameplayShouldRun=false;window.BerriesYandex?.gameplayStop?.();this.kingAnim('celebrate');this.fx.win();const sv=loadSave(),done=new Set(sv.done||[]);done.add(this.no);sv.done=[...done];sv.coins=(sv.coins||0)+(this.no<=5?80:this.no<=10?100:this.no<=15?120:this.no<=20?140:this.no<=25?160:180);saveLocal(sv);this.resultPopup(true,sv)}
  async lose(){if(this.busy)return;this.busy=true;clearTimeout(this.hintTimer);window.__berriesGameplayShouldRun=false;window.BerriesYandex?.gameplayStop?.();this.kingAnim('sad');this.fx.lose();this.resultPopup(false,loadSave())}
