...
class Sfx{
  ...
  stopAll(){for(const timer of this.berryTimers||[])timer.remove(false);this.berryTimers?.clear();for(const timer of this.sampleTimers||[])timer.remove(false);this.sampleTimers?.clear();for(const pool of Object.values(this.voices))for(const sound of pool)sound.stop();for(const node of this.nodes){try{node.stop()}catch{}}this.nodes.clear()}
  sample(key,volume=.5,detune=0,seek=0,durationMs=0){
    ...
    voice.play({volume,detune,seek});
    if(durationMs>0){
      this.sampleTimers??=new Set();
      const timer=this.s.time.delayedCall(durationMs,()=>{
  ...
  acornCrunch(){
    if(!this.can('acorn-crunch',120))return;
    // The uploaded Pixabay source contains several crunch accents.
    // Use only the middle dry crunch around 1.1 s; stop before the final accent.
    if(this.sample('sfx_acorn_crunch',.62,0,.98,780))return;
    this.wood();
  }
  berryPop(chain=1){
...
class Play extends Phaser.Scene{
  ...
  async clearCells(initial,chain=1){const set=this.expandSpecials(new Set(initial)),damageRoots=new Set(),damageAcorns=new Set(),damagedIce=new Set();for(const key of set){const [r,c]=key.split(',').map(Number),it=this.board[r][c];if(it?.sp)this.music?.duck?.(420,it.sp==='bomb'?.32:.42);if(it?.sp==='line_h'){this.fx.specialLine('h');this.lineFx(r,c,'h')}if(it?.sp==='line_v'){this.fx.specialLine('v');this.lineFx(r,c,'v')}if(it?.sp==='bomb'){this.fx.bomb();this.bombFx(r,c)}if(it?.sp==='rainbow')this.rainbowFx();if(this.cell[r][c].block==='roots')damageRoots.add(key);if(this.cell[r][c].block==='acorn')damageAcorns.add(key);for(const [dr,dc] of [[1,0],[-1,0],[0,1],[0,-1]]){const rr=r+dr,cc=c+dc;if(!inBounds(rr,cc))continue;if(this.cell[rr][cc].block==='roots')damageRoots.add(`${rr},${cc}`);if(this.cell[rr][cc].block==='acorn')damageAcorns.add(`${rr},${cc}`)}}
    ...
    for(const key of damageRoots){const [r,c]=key.split(',').map(Number);if(this.cell[r][c].block==='roots'){this.cell[r][c].block=null;this.bumpGoal('roots',null,1);this.fx.vinesBreak();const p=this.pos(r,c);this.burst(p.x,p.y,0x9c6b3c,10);this.render(r,c)}}
    for(const key of damageAcorns){const [r,c]=key.split(',').map(Number);if(damagedIce.has(key))continue;if(this.cell[r][c].block==='acorn'&&this.cell[r][c].ice>0){this.cell[r][c].ice--;if(!this.cell[r][c].ice)this.bumpGoal('ice',null,1);this.fx.iceBreak();const p=this.pos(r,c);this.iceShards(p.x,p.y);this.render(r,c);continue}if(this.cell[r][c].block==='acorn'&&!this.cell[r][c].ice){this.cell[r][c].block=null;this.bumpGoal('acorn',null,1);(this.fx.acornCrunch?this.fx.acornCrunch():this.fx.spark?.());const p=this.pos(r,c);this.burst(p.x,p.y,0xffe48d,10);this.render(r,c)}}this.updateHud();if(chain>=2)this.kingReact('celebrate',520)}
