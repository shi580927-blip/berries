...
## 2026-09-24 — UI STANDARD: динамический текст и цены в поп-апах

Обязательный production-стандарт для «Безумных ягодок» и шаблон для следующих игровых проектов:

- не рисовать в фоне поп-апа готовые поля/плашки под динамические цены, количества, статусы и локализуемые тексты;
- фон/рамка поп-апа содержит только постоянный декоративный арт;
- интерактивные кнопки делаются отдельными production-ассетами (выпуклые, в стилистике игры);
- цена, количество, статус покупки и другие меняющиеся значения выводятся кодом поверх кнопки;
- текст всегда привязан к центру своей кнопки/слота, а не к абсолютной позиции на фоне;
- для текста задать safe width, center alignment, max font size и автоматическое уменьшение при переполнении;
- для двухстрочных подписей использовать фиксированный line spacing и вертикальное центрирование;
- EN/TR и другие локализации должны использовать те же кнопки без перерисовки фона;
- перед релизом проверять кнопки/тексты на desktop + Android + iPhone/iPad и разных viewport;
- если элемент предполагает цену от SDK, цена никогда не должна быть частью PNG.

Текущие магазинные поп-апы в v9 считаются рабочими, но подлежат будущей production-перерисовке по этому стандарту после модерации.
